	...
	_render:function(){
		...
		try{
			$('#templateManager').load(laurbe.templateManager.templatePath+appLayoutTemplate.url, function(templateString,  ajaxObject, ajaxState){
				console.log('se supone que esoty dentro');
			...
			console.log('hay error en jquery '+e);
		}
	},
	...
	**/
	_buildMenu:function(){
		...
		this.menu = new laurbe.NavBar({	
										...
										items:menuItems,
          								brand:this.instanceProperties.navBar? this.instanceProperties.navBar.brand:null,
          								searchTool:this.instanceProperties.navBar?this.instanceProperties.navBar.searchTool:null

		});
		
		if(this.instanceProperties.bottomNavBar){
